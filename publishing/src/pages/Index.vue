    	...
    	<div id="gmatListTop" class="container">
		  ...
		  <br>
			<tabs type="primary">
				...
				<tab-pane label="내친구 맛집">
					<div class="row">
						<card v-for="item in friendsReviewData" v-bind:key="item.id" style="width: 100%;">
							<ul slot="raw-content" class="list-group list-group-flush">
								<li class="list-group-item">
									<p style="font-size:24px;"><a v-on:click="goRestaurantDetail(item.store_id)">{{item.store_name}}</a>
		  ...
		  axios.get('/api/gmatList.php')
				  .then(function(response){
					  ...
					  if(response.data.code == '1') {
						  ...
						  for(let i = 0; i < response.data.friendsdata.length; i++){
							  vm.friendsReviewData.push({
								  store_id: response.data.friendsdata[i].store_id,
								  store_name: response.data.friendsdata[i].store_name,
								  category: response.data.friendsdata[i].category,
								  telephone: response.data.friendsdata[i].telephone,
								  address: response.data.friendsdata[i].address,
								  roadaddress: response.data.friendsdata[i].roadaddress,
								  review_seq: response.data.friendsdata[i].review_seq,
								  lon: response.data.friendsdata[i].lon,
								  lat: response.data.friendsdata[i].lat,
								  cnt: response.data.friendsdata[i].cnt,
								  ratingav: Number.parseFloat(response.data.friendsdata[i].ratingav).toFixed(2)
							  })
						  }
...
// 마커의 이미지정보를 가지고 있는 마커이미지를 생성합니다
	  ...
	  makeMarkerFriendsReview() {
		  for (let i = 0; i < this.friendsReviewData.length; i++) {
			  var imageSrc = 'img/marker/friend_review.png', // 마커이미지의 주소입니다
					  imageSize = new kakao.maps.Size(20, 25), // 마커이미지의 크기입니다
					  imageOption = {offset: new kakao.maps.Point(45, 40)}; // 마커이미지의 옵션입니다. 마커의 좌표와 일치시킬 이미지 안에서의 좌표를 설정합니다.
// 마커의 이미지정보를 가지고 있는 마커이미지를 생성합니다
			  var markerImage = new kakao.maps.MarkerImage(imageSrc, imageSize, imageOption),
					  markerPosition = new kakao.maps.LatLng(this.friendsReviewData[i].lat, this.friendsReviewData[i].lon); // 마커가 표시될 위치입니다
			  ...
			  var content = '<div class="wrap" id="overF' + i + '" style="display:none;">' +
					  ...
					  '            친구가 등록한 맛집' +
					  '            <div class="close" onclick="document.getElementById(\'overF' + i + '\').style.display=\'none\';" title="닫기"></div>' +
					  '        </div>' +
					  '        <div class="body">' +
					  '            <!--div class="img">' +
					  '                <img src="http://cfile181.uf.daum.net/image/250649365602043421936D" width="73" height="70">' +
					  '           </div-->' +
					  '            <div class="desc">' +
					  '                <div class="ellipsis" style="color:black;text-decoration:underline;" onclick="location.href=\'/#/restaurant/' + this.friendsReviewData[i].store_id + '\'"><b>' + this.friendsReviewData[i].store_name + '</b><br/> <a onclick="location.href=\'/#/restaurant/' + this.friendsReviewData[i].store_id + '\'" href="#" style="color:gray;"> (리뷰/찜하기 개수 : ' + this.friendsReviewData[i].cnt + '개)</a></div>' +
					  '                <div class="ellipsis" style="color:gray;">' + this.friendsReviewData[i].category + '</div>' +
					  '                <div class="ellipsis" style="color:black;"> 평균별점 ' + othersRatingImg + ' (' + this.friendsReviewData[i].ratingav + '점)</div>' +
					  '                <!--div><a href="http://www.kakaocorp.com/main" target="_blank" class="link">홈페이지</a></div-->' +
					  '            </div>' +
					  '        </div>' +
					  '    </div>' +
